...
const Details = () => {
    ...
    const {content, isLoading: contentIsLoading}=useFetch(`/${mediaType}/${id}`,[id])
  ...
  const [backdropImage,setBackground]=useState()
  ...
  useEffect(()=>{
    const fetchBackgroundImage = async()=>{
      try {
        setIsLoading(true)
        const backgroundImageUrl = `${url?.backdrop ?? ''}${content?.backdrop_path ?? ''}`;
  ...
  return (
    ...
    {contentIsLoading ? (
        ...
        <>
            ...
            <img src={backdropImage} alt="" className='backdrop-image' />
            </div>
